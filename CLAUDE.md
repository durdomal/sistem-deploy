...
- **В конце каждого блока работы** дописывай одну запись СВЕРХУ в
  `..\HANDOVERS\sistem\code.md` по формату из `..\HANDOVERS\README.md`
  (Что сделано / Файлы / Что дальше / Ссылки). Старые записи не удалять.
- Крупное решение → отдельная запись в `..\HANDOVERS\sistem\decisions.md`.
- Когда Sistem MCP жив — параллельно `sistem_log_event(project_id, event)`.
